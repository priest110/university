...
tracer::scene model::loadobj(const std::string &objFileName) {
  ...
  auto ret = tinyobj::LoadObj(&attrib, &shapes, &obj_materials, &err,
                              objFileName.c_str(), obj_base_dir.c_str(), true);

  if (!ret || !err.empty()) {
    throw std::runtime_error("TinyOBJ Error loading " + objFileName +
                             " error: " + err);
  }
  ...
  for (size_t s = 0; s < shapes.size(); ++s) {
    ...
    auto minmax_matid = std::minmax_element(obj_mesh.material_ids.begin(),
                                            obj_mesh.material_ids.end());
    ...
    for (size_t f = 0; f < obj_mesh.num_face_vertices.size(); ++f) {
      if (obj_mesh.num_face_vertices[f] != 3) {
        throw std::runtime_error("Non-triangle face found in " + objFileName +
                                 "-" + shapes[s].name);
      }

      tracer::vec3<unsigned int> tri_indices;
      for (size_t i = 0; i < 3; ++i) {
        const tracer::vec3<int> idx(obj_mesh.indices[f * 3 + i].vertex_index,
                                    obj_mesh.indices[f * 3 + i].normal_index,
                                    obj_mesh.indices[f * 3 + i].texcoord_index);

        uint32_t vert_idx = obj.vertex.size();

        obj.vertex.emplace_back(attrib.vertices[3 * idx.x + 0],
                                attrib.vertices[3 * idx.x + 1],
                                attrib.vertices[3 * idx.x + 2]);

        if (idx.y != uint32_t(-1)) {
          tracer::vec3<float> n(attrib.normals[3 * idx.y],
        ...
        if (idx.z != uint32_t(-1)) {
          obj.uv.emplace_back(attrib.texcoords[2 * idx.z],
