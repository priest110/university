  ...
  #pragma omp parallel num_threads(num_threads)
  {
    ...
    do
    {
      ...
      for(i = 1; i < linhas-1; i++)
      {
        ...
        depend(in: m[i-1][j-1], m[i-1][j], m[i-1][j+1],m[i][j+1], m[i][j-1], m[i+1][j-1], m[i+1][j], m[i+1][j+1]) \
        depend(inout: m[i][j])
        for(j = 1; j < colunas-1; j++)
        {
          { 
            ...
            (
              ...
              n >= 2 &&
              s == 1 &&
              (m)[i][j+1] + !(m)[i+1][j] + !(m)[i-1][j] * !(m)[i][j-1] == 1
            )
            {
              (m)[i][j] = 0;
              changed = 1;
...
int main(int argc, char** argv){
  if(!argv[1])
    ...
    {   
      ...
      stopTime(iter);   
      write_matrix("output/snake_op_tasks.pgm", img); 
    }
    else
    {   
